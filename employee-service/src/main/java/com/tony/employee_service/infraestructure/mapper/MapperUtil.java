...
public interface MapperUtil {
  ...
  default BigDecimal bytesToBigDecimal(ByteBuffer bytes) {
    return bytes != null
        ? DECIMAL_CONVERSION.fromBytes(bytes, DECIMAL_SCHEMA, DECIMAL_SCHEMA.getLogicalType())
        : null;
  }

  default ByteBuffer bigDecimalToBytes(BigDecimal value) {
    return value != null
        ? DECIMAL_CONVERSION.toBytes(value, DECIMAL_SCHEMA, DECIMAL_SCHEMA.getLogicalType())
        : null;
  }
